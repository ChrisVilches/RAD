class Container < ApplicationRecord
  # TODO: This stuff is complex as fuck, and impossible to maintain/change.
  # Just turn it into a JSONB field and save it as a document in Postgres.
  # Also, I'd like query revisions to also hold a JSONB for the params config item, so
  # that's another reason to make it simpler.
  #
  # Also, adding new query versions (i.e. new ways of creating queries) will be easier
  # this way, and validators can be created easily.
  ...
  def to_debug_s
    def traverse(depth, container)
      ...
      elements = container.elements.sort_by(&:position)
      string = ''
    ...
    ret = "Created #{created_at}\n"
    ret << "Updated #{updated_at}\n"
    ret << traverse(0, self)
    ret
  end
  ...
  def find_element_from_path(path)
    ...
    curr.elements.find { |e| e.variable_name == path.last }
  end
  ...
  def all_variable_names_are_unique?
    ...
    elements.each_with_index do |elem, idx|
      ...
      if first_occurrences.key?(variable_name)
  ...
  def element_order_correct?
    elements = self.elements.sort_by(&:position)
